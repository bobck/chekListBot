...
async function maponGet() {
  const response = await fetch(
    `https://mapon.com/api/v1/unit/list.json?key=${process.env.MAPON_API_KEY}`,
    {
      method: 'GET',
    }
  );
...
export async function carMillegeByMaponIdOrOlateNumber({
  maponId,
  plateNumber,
}) {
  ...
  const foundByPlaneNumber = units.find(
    (unit) => transliterate(unit.number).replace(/\s/g, '') === plateNumber
  );
